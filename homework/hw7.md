...
```r
amniota <- readr::read_csv("/Users/aminamuhic/Desktop/BIS15W2024_amuhic/lab8/data/amniota.csv")
```
...
```r
amphibio <- readr::read_csv("/Users/aminamuhic/Desktop/BIS15W2024_amuhic/lab8/data/amphibio.csv")
```
...
##  Max.   :14610.0
```
_At a glance, it looks like NAs (for numerics) are represented with '-999.0'_
...
##  NA's   :1079
```
_It looks like NAs are represented appropriately (with NA) in many cases._
...
```r
amniota %>% 
  summarize(n_nas = sum(is.na(amniota)))
```
...
```r
# total number of NAs in `amniota` data
```

_It doesn't really make sense that the `amniota` data set has no NAs... it looks like outrageous numbers (i.e., -999.0) are used to represent NAs._


```r
amphibio %>% 
  summarize(n_nas = sum(is.na(amphibio)))
```
...
```r
# total numer of NAs in `amphibio` data
```

_This number of NAs makes more sense for the `amphibio` data set._
...
```
_Usually, NAs represent missing data. However, given that mammals don't lay eggs and that all of the `egg_mass_g` observations for mammals are NAs, NA here can also apply to non-applicable data._
...
## 4 Ter        1104     16.3
```
_I think that maybe NA, for these variables, was meant to be interpreted as 'no' (e.g., NA under the `Ter` variable = this amphibian is not terrestrial). The only other observation under these variables is '1'._

**10. Now that we know how NA's are represented in the `amniota` data, how would you load the data such that the values which represent NA's are automatically converted?**

```r
amniota_cleanload <- 
  readr::read_csv(file = "/Users/aminamuhic/Desktop/BIS15W2024_amuhic/lab8/data/amniota.csv", na = "-999")
```
