...
```{r}
mammals <- read_csv("data/mammal_lifehistories_v2.csv") %>% clean_names() # pipe clean names - clean names only cleans column heads, not the observations themselves!
```
...
mammals %>% 
  ...
  arrange(newborn) # mutate to create new variable (so you don't overwrite and mask changes) - if it looks into new variable and sees -999.0, it will be replaced with NA, anything else will stay the same
```
