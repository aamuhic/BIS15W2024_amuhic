...
## 6 Henry       88    87    88    72
```
_Not tidy data. You've made the column names an actual observation._
...
_The data are untidy because income should be a variable, where the income ranges are observations under that variable.
...
```
_`artist`, `track`, and `date` looks fine but we need to make weeks tidy._
...
_The `day#` variables should be observations under one variable._
...
```
## # A tibble: 90 × 5
##    genotype water_sched_prog greenhouse day       v
##    <chr>    <chr>            <chr>      <chr> <dbl>
