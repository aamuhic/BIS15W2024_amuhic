...
ui <- fluidPage(
    ...
    titlePanel("Log10 Homerange by Taxon"),
    
    selectInput("x", "Select Taxon", choices = c("lake fishes", "river fishes", "marine fishes", "birds", "mammals", "lizards", "snakes", "turtles", "tortoises"),selected = "mammals"),
...
server <- function(input, output) {
  ...
  output$plot <- renderPlot({
    ...
    homerange %>% filter(taxon==input$x) %>% ggplot(aes_string(x="log10.hra")) + geom_density(fill="cornflowerblue", color="black", alpha=0.4) + labs(x="Log10 Homerange", y="Density")
...
ui <- dashboardPage(
  ...
  dashboardBody(
  fluidRow(
  box(title = "Plot Options", width = 3, #change size of box
...
ui <- dashboardPage(
  ...
  box(title = "Plot Options", width = 3, #change size of box
    ...
    selectInput("x", "Select Population", choices = unique(wolves$pop), selected = "AK.PEN")

), # close the first box    

box(title = "Sex Composition", width = 9,
  plotOutput("plot", width = "800px", height = "600px")
) # close the second box
) # close the row
) # close the dashboard body
...
server <- function(input, output, session) {
  ...
  output$plot <- renderPlot({
    ...
    wolves %>% filter(pop==input$x) %>% filter(sex!="NA") %>% 
      ggplot(aes_string(x="sex", fill="sex")) + geom_bar()
