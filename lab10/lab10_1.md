...
_Bigger animals go further(have bigger home range). We're using log transformed values because some differences between animals are too large._
...
_`ggplot()` will automatically remove NAs, but you will get a warning message._
_if you want to see the warning message: `geom_point(na.rm = T)_
...
```r
# `geom_bar() is doing the counting for you
```
