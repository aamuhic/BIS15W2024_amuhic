...
```{r}
fisheries_tidy %>% 
  filter(isscaap_taxonomic_group=="Herrings, sardines, anchovies" & between(year, 1990, 2000)) %>% 
  group_by(country) %>% 
  summarize(all_caught=sum(catch, na.rm = TRUE)) %>% 
  arrange(desc(all_caught))
```
_Peru caught the most saradines between 1990-2000._
...
fisheries_tidy %>%
  ...
  arrange(desc(all_caught))
```
_India, Chins, Spain, Algeria, and France._
